...
let singlesWithSparkLineExpressions =
...
let graphPanels =
    ...
    -> map
        ...
        (indexed (List Text) graphPanelExpressions)
...
let singleStats =
    ...
    -> map
        ...
        (Natural -> Grafana.Panels.Panels)
        (\(expr : { index : Natural, value : List Text })
            -> makeSingleWithSparkLine chunk expr.index expr.value dim)
        (indexed (List Text) singlesWithSparkLineExpressions)
...
Grafana.Dashboard::{
    ...
    , panels = Grafana.Utils.generateIds
        ( concat (Natural -> Grafana.Panels.Panels)
            [ magic { chunks = 1, perChunk = 4 } { y = 0, w = 6, h = 4 } singleStats
            , magic { chunks = 1, perChunk = 4 } { y = 5, w = 6, h = 4 } singleStats
